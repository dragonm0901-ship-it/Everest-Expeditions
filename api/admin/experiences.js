import { listExperienceRecords, saveExperienceRecord } from '../_lib/database.js';
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      const records = listExperienceRecords();
      res.status(200).end(JSON.stringify({ experiences: records }));
    } catch (err) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const record = saveExperienceRecord(body);
      res.status(200).end(JSON.stringify({ experience: record }));
    } catch (err) {
      res.status(500).end(JSON.stringify({ message: err.message }));
    }
    return;
